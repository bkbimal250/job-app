  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        const [jobData, spasData, enumsResponse] = await Promise.all([
          jobService.getJobById(id),
          spaService.getAllSpas(),
        ...
        if (processedJobData.spa && typeof processedJobData.spa === 'object' && processedJobData.spa._id) {
          processedJobData.spa = processedJobData.spa._id;
        }
        
        // Handle category which might be an object or a string
        if (processedJobData.category && typeof processedJobData.category === 'object' && processedJobData.category.name) {
          processedJobData.category = processedJobData.category.name;
        }
        
        setFormData(processedJobData);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const cleanedData = {
        ...
        hrWhatsapp: formData.hrWhatsapp?.trim() || '',
        hrPhone: formData.hrPhone?.trim() || '',
        gender: formData.gender || 'Any',
        isNewJob: formData.isNewJob ?? true,
        vacancies: formData.vacancies ? parseInt(formData.vacancies, 10) : 1,
      ...
      if (!cleanedData.spa) {
        setError('Spa selection is required');
        setIsSubmitting(false);
        return;
      }
      ...
      await jobService.updateJob(id, cleanedData);
